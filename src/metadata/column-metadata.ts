...
export class ColumnMetadata {
  ...
  constructor(options: {
    ...
    if (options.args.options.readonly !== undefined) this.isReadonly = options.args.options.readonly;
    ...
    if (options.args.options.srid) this.srid = options.args.options.srid;
  ...
  createValueMap(value: any, useDatabaseName = false) {
    ...
    if (this.embeddedMetadata) {
      ...
      const extractEmbeddedColumnValue = (propertyNames: string[], map: ObjectLiteral): any => {
        ...
        if (this.generationStrategy === "increment" && this.type === "bigint") value = String(value);
  ...
  getEntityValue(entity: ObjectLiteral, transform: boolean = false): any | undefined {
      ...
      if (embeddedObject) {
        if (this.relationMetadata && this.referencedColumn) {
          ...
          if (relatedEntity && relatedEntity instanceof Object && !(relatedEntity instanceof FindOperator)) {
            ...
            !(embeddedObject[this.propertyName] instanceof FindOperator)
          ) {
    ...
    if (transform && this.transformer) value = this.transformer.to(value);
